...
{
	SunComponent = new TriangleComponent(CreateSphere(1.0, DirectX::SimpleMath::Vector4(1.0f, 1.0f, 0.0f, 1.0f)));
	Components.push_back(SunComponent);

	Planet = new TriangleComponent(CreateSphere(0.2, DirectX::SimpleMath::Vector4(0.8f, 0.5f, 1.0f, 1.0f), 5.0f, SunComponent));
	
	Planet->pos = { Planet->parent->pos.x + Planet->orbit, 0.0, 0.0 };
	Components.push_back(Planet);

	Sattelite = new TriangleComponent(CreateSphere(0.1, DirectX::SimpleMath::Vector4(1.0f, 0.5f, 0.4f, 1.0f), 1.0f, Planet));

	Sattelite->pos = { 0.0, Sattelite->parent->pos.y + Sattelite->orbit, 0.0 };
	Components.push_back(Sattelite);
}
...
void SolarSystemComponent::Update() 
{
	...
	SunComponent->rotate = { 0, angle, 0 };
	SunComponent->Update(Game::context, Game::camera.at(0));

	Planet->rotate = { 0, angle * 10.0f, 0 };
	Planet->pos = { 
		Planet->parent->pos.x + Planet->orbit * static_cast<float>(cos(angle * 5 * 3.14 / 180)),
		Planet->parent->pos.y,
		Planet->parent->pos.z + Planet->orbit * static_cast<float>(sin(angle * 5 * 3.14 / 180)) 
	};
